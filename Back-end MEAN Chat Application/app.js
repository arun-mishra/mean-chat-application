...
MongoClient.connect('mongodb+srv://Arun_Mishra:arun3040@cluster0.oxbb0.mongodb.net/Chat_App?retryWrites=true&w=majority', (err, Database) => {
    if(err) {
        console.log(err);
        return false;
    }
...
app.get('/chatroom/:room', (req, res, next) => {
    ...
    chatRooms.find({name: room}).toArray((err, chatroom) => {
        ...
        if(chatroom.length ==0){
            console.log('No chat room is created')
            return
        }
